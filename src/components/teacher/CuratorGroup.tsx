        ...
        >
          ...
          <Tabs defaultValue="info" className="w-full">
            <TabsList className={`grid w-full ${isMobile ? "grid-cols-2" : "grid-cols-5"}`}>
              <TabsTrigger value="info">{isMobile ? "Инфо" : "Основное"}</TabsTrigger>
              <TabsTrigger value="grades">{isMobile ? "Оценки" : "Успеваемость"}</TabsTrigger>
              {!isMobile && <TabsTrigger value="attendance">Посещаемость</TabsTrigger>}
              {!isMobile && <TabsTrigger value="works">Работы</TabsTrigger>}
              {!isMobile && <TabsTrigger value="comments">Отзывы</TabsTrigger>}
            </TabsList>

            <TabsContent value="info" className="space-y-4">
              <div className={`grid ${getFormCols()} gap-4`}>
                <div className="space-y-3">
                  <h4 className="font-medium">Личная информация</h4>
                  <div className="text-sm space-y-2">
                    <div><strong>ФИО:</strong> {selectedStudent.name}</div>
                    <div><strong>Группа:</strong> {currentGroup.name}</div>
            ...
            <TabsContent value="grades" className="space-y-4">
              <div className="text-center py-8">
                ...
                <h3 className="font-medium mb-2">Успеваемость</h3>
                <p className="text-sm text-gray-500">
                  Средний балл: {getStudentStats(selectedStudent.id).averageGrade}
                </p>
              </div>
            </TabsContent>
          </Tabs>
